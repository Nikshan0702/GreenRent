...
// import multer from "multer";
// import fs from "fs";
// import PropertyModel, { computeCompositeBadge } from "../Models/Property.js";
// import authenticateUser from "../middleware/authenticateUser.js";
// import { getVisionClient } from "../lib/vision.js";
...
// const fileFilter = (_req, file, cb) => {
//   const ok = (file.mimetype && file.mimetype.startsWith("image/")) || file.mimetype === "application/pdf";
//   cb(ok ? null : new Error("Only images or PDFs are allowed"), ok);
// };
...
// /** Parse OCR text → issuer/scheme/rating and derive a certificate badge */
// const TRUSTED_ISSUERS = [
//   /IFC\s*EDGE/i,
//   /EDGE\s+Buildings/i,
//   /USGBC|GBCI/i,
...
//   const issueDate = found(/(?:Issued\s*On|Issue\s*Date)[:\s]*([0-9]{4}[-\/][0-9]{2}[-\/][0-9]{2})/i) || "";
//   const validUntil = found(/(?:Valid\s*Until|Expiry\s*Date)[:\s]*([0-9]{4}[-\/][0-9]{2}[-\/][0-9]{2})/i) || "";
...
//   let certBadge = "Unverified";
//   if (scheme === "EDGE") {
//     if (/Zero\s*Carbon/i.test(ratingRaw)) certBadge = "Platinum";
//     else if (/Advanced/i.test(ratingRaw)) certBadge = "Gold";
//     else if (/Certified/i.test(ratingRaw)) certBadge = "Silver";
//   } else if (scheme === "GreenSL" || scheme === "LEED") {
//     if (/Platinum/i.test(ratingRaw)) certBadge = "Platinum";
//     else if (/Gold/i.test(ratingRaw)) certBadge = "Gold";
//     else if (/Silver/i.test(ratingRaw)) certBadge = "Silver";
//     else if (/Certified|Bronze/i.test(ratingRaw)) certBadge = "Bronze";
//   } else if (scheme === "BREEAM") {
//     if (/Outstanding/i.test(ratingRaw)) certBadge = "Platinum";
//     else if (/Excellent/i.test(ratingRaw)) certBadge = "Gold";
//     else if (/Very\s*Good/i.test(ratingRaw)) certBadge = "Silver";
//     else if (/Good|Pass/i.test(ratingRaw)) certBadge = "Bronze";
//   } else if (scheme === "EPC") {
//     if (/A\+/i.test(ratingRaw)) certBadge = "Platinum";
//     else if (/^A$/i.test(ratingRaw)) certBadge = "Gold";
//     else if (/^B$/i.test(ratingRaw)) certBadge = "Silver";
//     else if (/^C$/i.test(ratingRaw)) certBadge = "Bronze";
//   } else {
//     if (/Platinum|A\+/i.test(ratingRaw)) certBadge = "Platinum";
//     else if (/Gold|^A$/i.test(ratingRaw)) certBadge = "Gold";
//     else if (/Silver|^B$/i.test(ratingRaw)) certBadge = "Silver";
//     else if (/Bronze|^C$/i.test(ratingRaw)) certBadge = "Bronze";
//   }

//   // issuer trust check
//   const issuerTrusted = TRUSTED_ISSUERS.some((re) => re.test(issuer));
//   if (!issuerTrusted && certBadge !== "Unverified") {
//     certBadge = "Unverified";
//   }

//   const confidence = certBadge === "Unverified" ? 0.5 : scheme ? 0.9 : 0.85;

//   return {
//     fields: { scheme, rating: ratingRaw, issuer: issuer?.trim(), certId, issueDate, validUntil },
//     certBadge,
//     confidence,
//   };
// }

// /** POST /CertificateOperations/ocr/:propertyId  (multipart field: certificate) */
// router.post("/ocr/:propertyId", authenticateUser, upload.single("certificate"), async (req, res) => {
//   const cleanup = () => {
//     if (req.file?.path && fs.existsSync(req.file.path)) {
//       try { fs.unlinkSync(req.file.path); } catch {}
...
//     } catch (e) {
//       cleanup();
//       return res.status(500).json({ success: false, message: "Vision client not configured." });
//     }
...
//     prop.ecoCertificate = {
//       filePath: req.file.path,  // keep file for audit
//       ocrText: fullText,
//       issuer: parsed.fields.issuer,
//       certId: parsed.fields.certId,
//       issueDate: parsed.fields.issueDate,
//       validUntil: parsed.fields.validUntil,
//       ratingRaw: parsed.fields.rating,
//       badge: parsed.certBadge,   // ⭐ store cert badge
//       parsedAt: new Date(),
//       confidence: parsed.confidence,
//     };

//     // Composite badge = stronger( featuresBadge , certificate.badge )
//     prop.ecoBadge = computeCompositeBadge(prop.ecoFeatures, parsed.certBadge);
...
//   } catch (err) {
//     console.error("[Certificate OCR] error:", err?.message || err);
//     // keep the uploaded file on success; delete on failure
//     try {
//       if (req.file?.path && fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
//     } catch {}
//     return res.status(500).json({ success: false, message: "OCR failed" });
...
/** Simple parser → issuer/rating/badge (same logic you used earlier, shortened) */
const TRUSTED_ISSUERS = [
  /IFC\s*EDGE/i,
  /EDGE\s+Buildings/i,
  /USGBC|GBCI/i,
...
function parseCertificateText(fullText = "") {
  ...
  if (has(/\bEDGE\b/i)) {
    scheme = "EDGE";
    if (has(/Zero\s*Carbon/i)) { ratingRaw = "EDGE Zero Carbon"; badge = "Platinum"; }
    else if (has(/Advanced/i)) { ratingRaw = "EDGE Advanced"; badge = "Gold"; }
    else { ratingRaw = "EDGE Certified"; badge = "Silver"; }
  } else if (has(/\bGreen\s*SL\b|\bGreenSL\b|\bGBCSL\b/i)) {
    scheme = "GreenSL";
    ratingRaw = found(/\b(Platinum|Gold|Silver|Bronze)\b/i) || "Unspecified";
    if (/Platinum/i.test(ratingRaw)) badge = "Platinum";
    ...
    else if (/Bronze/i.test(ratingRaw)) badge = "Bronze";
  } else if (has(/\bLEED\b/i) || has(/\bUSGBC\b|\bGBCI\b/i)) {
    scheme = "LEED";
    ratingRaw = found(/\b(Platinum|Gold|Silver|Certified)\b/i) || "Certified";
    if (/Platinum/i.test(ratingRaw)) badge = "Platinum";
  ...
  const issuerTrusted = TRUSTED_ISSUERS.some((re) => re.test(issuer));
  if (!issuerTrusted && badge !== "Unverified") badge = "Unverified";

  const confidence = badge === "Unverified" ? 0.5 : scheme ? 0.9 : 0.85;
...
router.post("/ocr/:propertyId", authenticateUser, upload.single("certificate"), async (req, res) => {
  ...
  try {
    ...
    // pre('save') will recompute ecoScore & ecoBadge (composite with cert.badge)
    await prop.save();
    ...
    return res.json({
      ...
      data: {
        ...
        ecoBadge: prop.ecoBadge,
        certificate: prop.ecoCertificate,
        ecoScore: prop.ecoScore,
        ecoFeatures: prop.ecoFeatures,
      },
    });
  } catch (err) {
    console.error("[Certificate OCR] error:", err?.message || err);
    try { // cleanup on hard failure
      if (req.file?.path && fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
    } catch {}
    return res.status(500).json({ success: false, message: "OCR failed" });
  }
...
/** DELETE /CertificateOperations/:propertyId
 *  - removes certificate and recomputes badge fallback from ecoScore
 */
router.delete("/:propertyId", authenticateUser, async (req, res) => {
  try {
    const { propertyId } = req.params;
    const prop = await PropertyModel.findById(propertyId);
    if (!prop) return res.status(404).json({ success: false, message: "Property not found" });
    ...
    const fp = prop.ecoCertificate?.filePath;
    prop.ecoCertificate = undefined;

    await prop.save(); // pre('save') recomputes ecoBadge fallback
