...
// Eco band colors (unchanged logic)
function getEcoBand(rating = 0) {
  if (rating >= 80) {
    return { label: 'Platinum Eco', bg: 'rgba(16,185,129,0.18)', border: '#34d399', text: '#065f46' };
  }
  if (rating >= 60) {
    return { label: 'Gold Eco', bg: 'rgba(245,158,11,0.16)', border: '#fbbf24', text: '#92400e' };
  }
  return { label: 'Standard Eco', bg: 'rgba(239,68,68,0.16)', border: '#f87171', text: '#7f1d1d' };
}
...
export default function ApartmentCarousel({
  ...
  // demo slides fallback (unchanged logic)
  const slides = useMemo(() => {
    if (data?.length) return data;
    return [
      { id: 'demo-1', title: 'Green View Apartments', location: 'Kollupitiya, Colombo', image: FallbackImage,  ecoRating: 82, rent: 'Rs. 120,000 / mo', bedrooms: 2 },
      { id: 'demo-2', title: 'City Eco Loft',        location: 'Nugegoda',            image: FallbackImage1, ecoRating: 68, rent: 'Rs. 95,000 / mo',  bedrooms: 1 },
      { id: 'demo-3', title: 'Sunrise Villas',       location: 'Rajagiriya',          image: FallbackImage2, ecoRating: 54, rent: 'Rs. 150,000 / mo', bedrooms: 3 },
    ];
  }, [data]);

  const renderItem = ({ item }) => {
    const band = getEcoBand(item.ecoRating);
    const src = item?.image && (item.image.uri || typeof item.image === 'number') ? item.image : FallbackImage;
    ...
    return (
      <TouchableOpacity
        ...
        accessibilityLabel={`Open ${item?.title || 'apartment'}`}
        style={{
          elevation: 2,
          shadowColor: '#000',
      ...
      >
        ...
        {/* Bottom info bar */}
        <View
          className="absolute bottom-0 left-0 right-0 flex-row items-center px-3.5 py-3"
          style={{
            backgroundColor: 'rgba(60,193,114,0.90)', // GREEN with stronger opacity for legibility
          }}
        >
          ...
          <View className="px-2.5 py-1 rounded-full bg-white/15 border border-white/35">
            <Text className="text-white text-xs font-bold">{item.bedrooms ?? '-'} BR</Text>
          </View>
        </View>
      </TouchableOpacity>
  ...
  return (
    <View className="my-4">
      <Carousel
        ...
        autoPlayInterval={autoPlayInterval}
        style={{ alignSelf: 'center' }}
        onProgressChange={(_, absoluteProgress) => setIndex(Math.round(absoluteProgress))}
      />

      {/* Pagination dots */}
      <View className="mt-3 self-center flex-row gap-1.5">
        {slides.map((_, i) => {
          const active = i === index % slides.length;
          return (
            <View
              key={`dot-${i}`}
              className={`h-2 rounded-full ${active ? '' : 'bg-gray-200'}`}
              style={{
                width: active ? 16 : 8,
                backgroundColor: active ? GREEN : undefined,
                transform: [{ scale: active ? 1 : 1 }],
              }}
              accessibilityLabel={active ? `Slide ${i + 1}, current` : `Slide ${i + 1}`}
            />
          );
        })}
      </View>
