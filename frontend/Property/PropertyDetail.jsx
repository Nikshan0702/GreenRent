...
export default function PropertyDetail() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!token || !prop?._id) return;
        const res = await fetch(`${API_BASE}/UserOperations/wishlist`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json().catch(() => ({}));
        const ids = Array.isArray(data?.data) ? data.data.map((p) => p._id) : [];
        setIsSaved(ids.includes(prop._id));
      } catch {}
    })();
  }, [prop?._id]);
  ...
  const toggleSave = useCallback(async () => {
    try {
      ...
      const url = `${API_BASE}/UserOperations/wishlist/${prop._id}`;
      const method = isSaved ? 'DELETE' : 'POST';
      const res = await fetch(url, { method, headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || data?.success === false) throw new Error(data?.message || 'Failed to update wishlist');
      setIsSaved(!isSaved);
    } catch (e) {
      ...
      >
        ...
        <View className="px-4 mt-4">
          ...
          <View className="flex-row mt-3" style={{ gap: 8 }}>
            ...
            >
              <Text className="text-[12px] font-semibold" style={{ color: INK }}>Compare</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View className="px-4 mt-5">
          ...
          >
            <View className="flex-row items-start justify-between">
              <Text className="text-xl font-extrabold flex-1 pr-3" style={{ color: INK }} numberOfLines={2}>
                {prop?.title || 'Property'}
              </Text>
              <TouchableOpacity
                onPress={toggleSave}
                className="rounded-xl items-center justify-center"
                style={{ paddingVertical: 6, paddingHorizontal: 10, backgroundColor: isSaved ? '#ECFDF5' : '#F3F4F6' }}
                accessibilityRole="button"
                accessibilityLabel={isSaved ? 'Remove from wishlist' : 'Save to wishlist'}
                activeOpacity={0.9}
              >
                <Ionicons name={isSaved ? 'heart' : 'heart-outline'} size={18} color={isSaved ? PRIMARY : INK} />
              </TouchableOpacity>
            </View>
