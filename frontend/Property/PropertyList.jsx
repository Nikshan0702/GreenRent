...
// /* ---------- Utility: derive a short location label ---------- */
...
//   const onCardPress = () => {
//     // Toggle expand/collapse for ALL cards (as requested)
//     setExpandAll((prev) => !prev);
//   };

//   const renderItem = ({ item, index }) => {
//     const img = ensureAbsolute(item?.images?.[0]?.url || item?.images?.[0]);
//     const expanded = expandAll; // all cards follow the single toggle
...
//             {/* concise location name beside pin */}
...
//         {/* expanded section (applies to all when expandAll = true) */}
//         {expanded && (
...
//   // 🔁 trigger to refresh CommentsBlock when a comment is posted/edited/deleted
...
// /* ---------- Comments block (compact list) ---------- */
// function CommentsBlock({ propertyId, compact = false, reloadKey }) {
//   const [items, setItems] = useState(null);
//   const [loading, setLoading] = useState(false);
...
//       if (!text.trim()) {
//         return Alert.alert('Required', 'Please enter your comment.');
//       }

//       const numRating = Number(rating);
//       if (Number.isNaN(numRating) || numRating < 1 || numRating > 5) {
//         return Alert.alert('Invalid rating', 'Rating must be between 1 and 5.');
//       }
...
//           Authorization: `Bearer ${token}`,
//         },
//         body: JSON.stringify({ text: text.trim(), rating: numRating }),
//       });
//       const data = await res.json();
//       if (!res.ok) throw new Error(data?.message || 'Comment failed');
...
//             multiline
//             className="bg-gray-100 rounded-xl px-3 py-2 h-24 mb-2"
//           />
//           <TextInput
//             value={rating}
...
//             className="bg-gray-100 rounded-xl px-3 py-2 mb-3"
//           />
//           <View className="flex-row">
//             <Pressable onPress={() => setModal({ visible: false, property: null })} className="flex-1 mr-2 bg-gray-200 rounded-xl py-3 items-center">
//               <Text className="font-semibold text-gray-800">Cancel</Text>
