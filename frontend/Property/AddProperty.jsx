...
export default function AddProperty({ navigation }) {
 ...
 // screens/AddProperty.jsx - Replace your handleSubmit function
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Convert blob URIs to actual files before appending
    for (let i = 0; i < images.length; i++) {
      const img = images[i];
      
      // If it's a blob URI, we need to convert it
      if (img.uri.startsWith('blob:')) {
        console.log('Converting blob URI to file:', img.uri);
        ...
        // Regular file URI (for physical devices)
        fd.append('images', {
          uri: img.uri,
